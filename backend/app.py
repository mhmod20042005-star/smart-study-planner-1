...
def normalize_text(value):
    """Unicode-aware normalization that preserves Hebrew, Arabic, English, and digits.
 
    Steps:
      1. NFC-normalize so composite characters are canonical.
      2. Lowercase (affects only Latin; Hebrew/Arabic are caseless).
      3. Strip leading/trailing whitespace.
      4. Remove characters that are not:
           - Latin letters  (category L*)
           - Hebrew letters (U+05D0–U+05EA and U+FB1D–U+FB4E)
           - Arabic letters (U+0600–U+06FF and U+0750–U+077F)
           - Decimal digits (category Nd)
           - Space
      5. Collapse multiple spaces to one.
    """
    v = unicodedata.normalize("NFC", value).lower().strip()
    # Keep: letters of any script (Lu/Ll/Lt/Lm/Lo), digits, space
    # Remove: punctuation, symbols, control chars
    v = "".join(
        ch if (unicodedata.category(ch).startswith("L") or
               unicodedata.category(ch) == "Nd" or
               ch == " ")
        else " "
        for ch in v
    )
...
def ensure_indexes():
    """Unique index on institution_normalized + course_code_normalized,
    but ONLY when both fields are present and non-empty.
    Uses partialFilterExpression so legacy/empty-string docs are never indexed."""
    try:
    ...
    try:
        db.courses.create_index(
            [("institution_normalized", 1), ("course_code_normalized", 1)],
            unique=True,
            name="uniq_inst_code",
            partialFilterExpression={
                "institution_normalized": {"$type": "string", "$gt": ""},
                "course_code_normalized": {"$type": "string", "$gt": ""},
            },
        )
    except Exception:
        pass
...
def add_course():
    ...
    if db.courses.find_one({"username": username, "course": course,
...
@app.route("/hub/report", methods=["POST"])
def hub_report():
    """Log a content report."""
    data        = request.json or {}
    ...
    if not report_type or not item_id or not reported_by:
        return jsonify({"error": "type, item_id, and reported_by are required"}), 400
 
    db.hub_reports.insert_one({
        "type":        report_type,
        "item_id":     item_id,
